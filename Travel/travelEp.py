...
from Travel.travelInstances import TravelInstance
...
def fetchExpensesForATrip():
    ...
    try:
        if tripUserHandler.checkDbConnection():
            ...
            return jsonify({"Message": tripUserService.fetchExpensesForTrip(tripId)}), 200
        else:
            return jsonify({"Error": "DB Connection Failed"}), 501
    except Exception as ex:
...
def deleteExpenseForTrip():
    ...
    try:
        if tripUserHandler.checkDbConnection():
            ...
            return jsonify({"Message": tripUserService.deleteExpenseFromTrip(expenseId)}), 200
        else:
            return jsonify({"Error": "DB Connection Failed"}), 501
    except Exception as ex:
        return jsonify({"Error": f"Error fetching all expenses for trip{ex}"}), 500
...
def addExpenseForTrip():
    ...
    try:
        if tripUserHandler.checkDbConnection():
            logging.info("-----Deleting expense for a trip-----")
            return jsonify({"Message": tripUserService.addExpenseForTrip(postedData['expense'])}), 200
        else:
            return jsonify({"Error": "DB Connection Failed"}), 501
    except Exception as ex:
        return jsonify({"Error": f"Error fetching all expenses for trip{ex}"}), 500
